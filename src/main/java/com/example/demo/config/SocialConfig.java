...
public class SocialConfig {
    ...
    private static final String TELEGRAM_CHAT_ID = "telegram.chatId";
    private static final String TELEGRAM_BOT_TOKEN = "telegram.botToken";

    private static final String TELEGRAM_POST_URL = "https://api.telegram.org/bot%s/sendMessage?chat_id=%s&text=";
    ...
    @Bean
    public TelegramClient getTelegramClient(Environment environment){
        String url = String.format(TELEGRAM_POST_URL,
                environment.getProperty(TELEGRAM_BOT_TOKEN),
                environment.getProperty(TELEGRAM_CHAT_ID));
        return new TelegramClient(url, getRestTemplate());
    }
}
